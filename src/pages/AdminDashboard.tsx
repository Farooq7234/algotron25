...
import { Users, Calendar, CreditCard, TicketCheck, UserPlus, User, QrCode, Award } from "lucide-react"
  ...
  useEffect(() => {
    const loadData = async () => {
      if (currentUser && isAdmin) {
        ...
        // Count payment statuses and team/individual registrations
        Object.values(usersData || {}).forEach((user: any) => {
          if (user.payment) {
            if (user.payment.status === "pending") {
              ...
              // Count team vs individual registrations
              if (user.payment.teamMember) {
                teamRegistrations++
              } else {
          ...
          // Count total attendance (entry QR scanned)
          if (
            user.tickets &&
            ((user.tickets.entry && user.tickets.entry.status === "Participated") ||
              (user.tickets.entry_team && user.tickets.entry_team.status === "Participated"))
          ) {
            totalAttendance++
          }

          // Count total participants in events (excluding food and entry)
          if (user.tickets) {
            Object.entries(user.tickets).forEach(([ticketKey, ticketData]: [string, any]) => {
              if (
                ticketKey !== "food" &&
                ticketKey !== "foodTeam" &&
                ticketKey !== "entry" &&
                ticketKey !== "entry_team" &&
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="container mx-auto px-4 py-8 mt-16">
        ...
        {/* New statistics cards for team/individual registrations and attendance */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-10">
          <NeonCard type="tech" className="p-6">
            <div className="flex items-center">
              <div className="mr-4 p-3 rounded-full bg-blue-900 bg-opacity-50">
                <UserPlus className="h-6 w-6 text-blue-400" />
              </div>
              <div>
                <p className="text-gray-400 text-sm">Team Registrations</p>
                <h3 className="text-2xl font-bold">{summary.teamRegistrations}</h3>
              </div>
            </div>
          </NeonCard>
          ...
          <NeonCard type="nontech" className="p-6">
            <div className="flex items-center">
              <div className="mr-4 p-3 rounded-full bg-pink-900 bg-opacity-50">
                <User className="h-6 w-6 text-pink-400" />
              </div>
              <div>
                <p className="text-gray-400 text-sm">Individual Registrations</p>
                <h3 className="text-2xl font-bold">{summary.individualRegistrations}</h3>
              </div>
            </div>
          </NeonCard>
          ...
          <NeonCard type="faculty" className="p-6">
            <div className="flex items-center">
              <div className="mr-4 p-3 rounded-full bg-teal-900 bg-opacity-50">
                <QrCode className="h-6 w-6 text-teal-400" />
              </div>
              <div>
                <p className="text-gray-400 text-sm">Total Attendance</p>
                <p className="text-xs text-gray-500">(Entry QR Scanned)</p>
                <h3 className="text-2xl font-bold">{summary.totalAttendance}</h3>
              </div>
            </div>
          </NeonCard>
          ...
          <NeonCard type="tech" className="p-6">
            <div className="flex items-center">
              <div className="mr-4 p-3 rounded-full bg-orange-900 bg-opacity-50">
                <Award className="h-6 w-6 text-orange-400" />
              </div>
              <div>
                <p className="text-gray-400 text-sm">Total Participants</p>
                <p className="text-xs text-gray-500">(Event QR Scanned)</p>
                <h3 className="text-2xl font-bold">{summary.totalParticipants}</h3>
              </div>
            </div>
          </NeonCard>
        </div>
          ...
          <NeonCard type="tech" className="p-6">
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={eventStats} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                  ...
                  />
                  <Bar dataKey="registrations" fill="#8884d8" isAnimationActive={true} animationDuration={1500} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </NeonCard>
              ...
              <ResponsiveContainer width="100%" height="100%">
                  ...
                  data={[
                    {
                      ...
                      count: eventStats
                        ...
                        .reduce((sum, e) => sum + e.registrations, 0),
                    },
                    {
                      name: "Non-Technical",
                      count: eventStats
                ...
                >
                  ...
                  <Tooltip
                    ...
                    labelStyle={{ color: "#fff" }}
                  />
                  <Bar dataKey="count" fill="#82ca9d" isAnimationActive={true} animationDuration={1500} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </NeonCard>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-gray-300">View and update event details, toggle registration status</p>
          </NeonCard>
          ...
          >
            ...
            <p className="text-gray-300">Review and approve payment requests, generate food tickets</p>
          </NeonCard>
          ...
          >
            ...
            <p className="text-gray-300">Scan tickets, mark attendance, track participation</p>
          </NeonCard>
        </div>
      </div>
    </div>
